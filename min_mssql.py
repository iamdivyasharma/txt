"""
vanna_min_mssql_bedrock.py
--------------------------
Ultra-small Vanna-style RAG for SQL Server using ONLY AWS Bedrock:
- Embeddings: Amazon Titan Text Embeddings V2 (Bedrock)
- LLM: Anthropic Claude 3 Haiku (Bedrock)

No Anthropic API key required. Uses your AWS creds/region (env, config, or IAM role).

Install:
    pip install pandas pyodbc chromadb boto3

Docs:
- Claude on Bedrock (InvokeModel, messages, anthropic_version): https://docs.aws.amazon.com/bedrock/latest/userguide/bedrock-runtime_example_bedrock-runtime_InvokeModel_AnthropicClaude_section.html
- Titan Embeddings V2 (dimensions/normalize): https://docs.aws.amazon.com/bedrock/latest/userguide/titan-embedding-models.html
"""
...
class BedrockTitanEmbeddingFunction:
    """
    Minimal embedding function wrapper for Chroma.
    Calls Bedrock 'bedrock-runtime' with modelId='amazon.titan-embed-text-v2:0'.
    """
    ...
    def __call__(self, texts: List[str]) -> List[List[float]]:
        ...
        for t in texts:
            ...
            if self.dimensions is not None:
                payload["dimensions"] = int(self.dimensions)   # e.g., 512
            if self.normalize is not None:
                payload["normalize"] = bool(self.normalize)    # True/False
...
def _get_info_schema(conn) -> pd.DataFrame:
    q = """
    SELECT TABLE_SCHEMA AS schema, TABLE_NAME AS table_name, COLUMN_NAME AS column_name,
           DATA_TYPE AS data_type, IS_NULLABLE AS is_nullable, ORDINAL_POSITION
...
def index_db(conn,
    ...
    """
    Build two Chroma collections with Titan V2 embeddings:
      - schema_chunks: one doc per table (DDL-like text)
      - row_samples: small JSON-lines chunks of real rows (random via NEWID())
    """
    import chromadb
    ...
    tables = (
        df[~df["schema"].isin(exclude_schemas)]
        .groupby(["schema","table_name"]).size().reset_index()[["schema","table_name"]]
    )
    ...
    for _, row in tables.iterrows():
        sch, tbl = row["schema"], row["table_name"]
        df_tbl = df[(df["schema"]==sch) & (df["table_name"]==tbl)]
...
def to_sql_with_bedrock_claude(question: str,
    ...
    """
    Calls Claude 3 Haiku on Bedrock (no Anthropic key).
    Uses the Messages API format with anthropic_version='bedrock-2023-05-31'.
    Returns the raw text Claude outputs (likely a code block).
    """
    import boto3
    ...
    body = json.loads(resp["body"].read())
    # Bedrock Claude returns a list of content blocks; take all text parts
    texts = []
