"""
vanna_min_mssql_bedrock_chat_fixed.py
-------------------------------------
Bedrock Chat Vanna-style RAG for Microsoft SQL Server:
- Titan embeddings via invoke_model
- Claude Haiku via bedrock-runtime.converse
Fix: avoid reserved keyword 'schema' by aliasing to table_schema in INFORMATION_SCHEMA query.
"""
...
def _get_info_schema(conn) -> pd.DataFrame:
    # Safe query to avoid reserved keyword issue
    q = """
    SELECT 
...
# ---------------- Claude Haiku via Bedrock Chat ----------------

def to_sql_with_bedrock_chat(question: str,
